...
`bun run describe <app-port.c | module.wasm> --pack <pack> --trace <t.json>`
drafts the observable two thirds of an app descriptor from a recorded
session (`tools/describe.ts`, and `docs/convention/app-bundle.md`'s "Drafting
a descriptor from a session"). It replays the trace, and attributes each
result to its input by REMOVING that input and replaying again, so what it
reports is what the press caused rather than what the app was doing anyway.
Out come `descriptor.draft.md` and a `descriptor.draft.json` with every
measurement behind every line; it never writes `descriptor.md`, and it leaves
every `(intent: ...)` and every `why` blank on purpose. `bun run
test:describe` is its proof, against the real chrono and fluidbox traces, and
`docs/decisions/0013-a-descriptor-is-measured-before-it-is-written.md` is why
this exists and what a draft cannot know.
...
test/describe/  proves tools/describe.ts against the real traces and the
                committed modules, no zig and no browser: that a session of
                chrono is enough to find its two button ROLES, its colour
                and its touch points, that a verdict computed from the
                draft matches the one computed from the hand-written
                descriptor on all three packs, and, by corrupting the
                trace two ways, that those assertions can be made to fail.
tools/          verify-bundle.ts (the listing verifier and actual
                ...
                as computeVerdict() for callers; --descriptor points it at
                a draft instead of the registered one), describe.ts (bun
                run describe: the observable two thirds of a descriptor
                drafted from a recorded session, each result attributed to
                its input by replaying the session again without it - see
                docs/decisions/0013), ledger.ts (bun run
...
apps/           portable app bundles, one per app: chrono/ (the reference
                ...
                original, reference/<donor>/ plus a NOTICE.md. chrono's and
                fluidbox's traces/ also carry a committed
                descriptor.draft.md and descriptor.draft.json: generated by
                bun run describe, never hand-edited, and never a
                descriptor.
