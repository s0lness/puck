...
import { computeButtonWiring } from "./buttonWiring";
...
//   BTN_BOOT / BTN_PWR    wasm/emu_shim.c's guarded PRIMARY button indices
//                         (app.h's app_frame_t carries one merged click and
//                         one merged key signal, and that contract does not
//                         change here), or -1 for a device that declares
//                         neither. BTN_CLICK[] / BTN_KEY[] carry EVERY
//                         declared click/key button, not just the primary
//                         one, so wasm/emu_shim.c can track a board's other
//                         buttons instead of silently ignoring them - see
//                         wasm/buttonWiring.ts for the assignment, which a
//                         board declaring four clicks and no key (Watchy,
//                         docs/convention/device-pack.md) is what forces to
//                         exist at all.
...
function generateDeviceHeader(devicePath: string): string {
  ...
  const wiring = computeButtonWiring(buttons);
  for (const i of wiring.keyIndices) {
    if (buttons[i]!.longPressMs === undefined) {
      ...
      // longPressMs delivers no KEY_SHORT at all. Checked for every
      // declared key, not only the primary one (BTN_PWR): a bystander key
      // button is still wired (BTN_KEY[], wasm/emu_shim.c) and would fail
      // the same way.
      console.error(`--device: ${devicePath} declares button "${buttons[i]!.id}" as role key with no longPressMs, so it can never deliver a short-press verdict`);
      cleanupRoster();
      process.exit(1);
    }
  }
  ...
  // BTN_CLICK[] / BTN_KEY[] always carry a trailing -1 sentinel so the array
  // is never zero-length (illegal in C) when a device declares no button of
  // that role at all (BTN_CLICK_COUNT / BTN_KEY_COUNT is the real count;
  // the sentinel is never read as a button index).
  const clickList = wiring.clickIndices.length > 0 ? `${wiring.clickIndices.join(", ")}, -1` : "-1";
  const keyList = wiring.keyIndices.length > 0 ? `${wiring.keyIndices.join(", ")}, -1` : "-1";
...
// afterwards. See that script's own header comment and wasm/buttonWiring.ts.
...
#define BTN_BOOT ${wiring.primaryClick}
#define BTN_PWR ${wiring.primaryKey}

// Every declared click/key button, not only the primary pair above - see
// wasm/emu_shim.c's own "every declared button" section for how a
// bystander (a Watchy's third and fourth click, say) gets tracked instead
// of silently dropped.
#define BTN_CLICK_COUNT ${wiring.clickIndices.length}
static const int BTN_CLICK[] = { ${clickList} };
#define BTN_KEY_COUNT ${wiring.keyIndices.length}
static const int BTN_KEY[] = { ${keyList} };
